...
const AllNotes = () => {
  ...
  const fetchNotes = async () => {
    setLoading(true);
    try {
      const response = await api.get("/notes/everynote");
    ...
    <div className="min-h-[calc(100vh-74px)] sm:py-10 sm:px-5 px-0 py-4">
      <div className="w-[92%] mx-auto ">
        {!loading && notes && notes?.length > 0 && (
          <h1 className="font-montserrat  text-slate-800 sm:text-4xl text-2xl font-semibold text-center">
            Latest Posts
          </h1>
        )}
        {loading ? (
            ...
            {notes && notes?.length === 0 ? (
              ...
              <>
                <div className="pt-10 grid xl:grid-cols-4 lg:grid-cols-3 sm:grid-cols-2 grid-cols-1 gap-y-10 gap-x-5 justify-center">
                  {notes.map((item) => (
                    <FeedNoteItem key={item.id} {...item} id={item.id} />
                  ))}
                </div>
              </>
            )}
